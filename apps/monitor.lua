...
function app.run(ctx)
 ...
 local function draw()
  ...
  local tw=math.max(10,math.min(22,w-4))
  ...
  if h>=18 then u.button(d,{x=2,y=h-2,w=tw,h=2,label='Done and Reboot'},false) end
 end
 draw()
 while true do
  local e=i.pull()
  if e.kind=='back' then os.reboot()
  elseif e.kind=='resize' then draw()
  elseif e.kind=='move' then if e.dir=='up' then sel=u.clamp(sel-1,1,math.max(1,#mons)) elseif e.dir=='down' then sel=u.clamp(sel+1,1,math.max(1,#mons)) end draw()
  elseif e.kind=='touch' then
   local w,h=d.getSize()
   if h>=18 and e.y>=h-2 then os.reboot()
   elseif e.y>=3 and e.y<5 then mode=mode=='screen' and 'keyboard' or 'screen' draw()
