...
local files = {"startup.lua","os/ui.lua","os/input.lua","apps/filemanager.lua","apps/settings.lua","apps/shell.lua","apps/printer.lua","apps/disk.lua","apps/monitor.lua"}
...
local function install(path)
  ...
  if not r then return false,tostring(err or "HTTP request failed") end
  local data=r.readAll() r.close()
  local dst="/"..path ensureDir(dst)
  if fs.exists(dst) then fs.delete(dst..".bak") fs.copy(dst,dst..".bak") end
  local f=fs.open(dst,"w") if not f then return false,"cannot write" end
  f.write(data) f.close() return true
end
term.clear() term.setCursorPos(1,1)
print("TouchOS Installer") print("GitHub: n3xus-unblocker/TouchOS") print("")
...
if count==#files then write("Reboot now? [y/n] ") if read():lower()=="y" then os.reboot() end else print("Some files failed. Check HTTP and try again.") end
